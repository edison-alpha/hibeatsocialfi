...
class PlaylistService {
  ...
  private async writePlaylistToBlockchain(playlist: Playlist, walletClient?: any): Promise<void> {
    try {
      ...
      // ✅ Use playlist ID as stream ID (like posts use postId)
      const streamId = playlist.id as Hex;
      
      // ✅ Debug data before encoding
      console.log(`📊 [PLAYLIST] Data to encode:`, {
        title: playlist.title,
        coverHash: playlist.coverHash,
        coverHashType: typeof playlist.coverHash,
        coverHashLength: playlist.coverHash?.length || 0,
      ...
      const encodedData = encoder.encodeData([
        ...
        { name: 'coverHash', value: playlist.coverHash || '', type: 'string' }, // ✅ Fallback to empty string
  ...
  async updatePlaylist(
    ...
    try {
      ...
      console.log(`✅ [PLAYLIST] Playlist updated successfully`);
    } catch (error) {
  ...
  async addTrackToPlaylist(playlistId: string, trackId: string, walletClient?: any): Promise<Playlist | null> {
    ...
    if (playlist.trackIds.includes(trackId)) {
      console.log(`⚠️ [PLAYLIST] Track already in playlist`);
      return playlist;
    }
